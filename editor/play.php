   ...
   <head>
    ...
    <style>
     ...
     <script>
        ...
        var strCurrentUser="舞台";
        var admin=-1;
        var bPlayer=true;
        var bWaiting=<?php echo ($admin==1)?"false":($bgcontrol==1?"true":"false");?>;

        var bCODECHANGED=false;

        var helpcontrol=0;		//初始化时控制互帮互助菜单的显示
        var sharecontrol=0;		//初始化时控制同组分享菜单的显示


        var trand=Math.random();				//翻译时间戳
        var arrUserList=Array();				//当前师生列表
        var bChattingRoom_NEWMESSAGE=false;
        var arrChattingRoom_MSGLIST=Array();

        var dataCollecting=false;
        var bREPORTING=true;
...
<div class="menu-bar_menu-bar-item_oLDa-"  id=NEWMESSAGE style="position:absolute; left:243px; top: 0px; width:80px; height: 48px; z-index:990;background-color: hsla(215, 100%, 65%, 1);font-size: 10px;color: red;"> </div> 
<!--div class="menu-bar_menu-bar-item_oLDa-" style="position:absolute; right:15px; top: 0px; line-height: 48px; height: 48px; z-index:990;background-color: hsla(215, 100%, 65%, 1);">同台竞技大舞台</div--> 
...
var arrPlayerCurrentCostume=Array();//当前项目中已存在的选手
...
function dealCommand(jsonCOMMAND)//接收到广播数据后，判断该执行什么操作。
{
   if(jsonCOMMAND.length>1)
   {
      ...
      case "INVITATION"://邀请时清空
         for(var i=oVMM.props.vm.runtime.targets.length-1;i>0 ;i--){			//初始化，清空所有角色
           if(oVMM.props.vm.runtime.targets[i].isStage!=true){
              oVMM.props.vm.deleteSprite(oVMM.props.vm.runtime.targets[i].id);
           }
         }
      ...
      case "NP"://投屏命令：["NP",\""+uname+"\",["+x+","+y+","+r+","+s+"],\""+f+"\","+c+"]

         /*
            计划改造：
                     初始化：   ["NP",\""+uname+"\",0,["SPRITE_FILE_MD5"]]
                     更新坐标： ["NP",\""+uname+"\",1,["+x+","+y+"]]  //(x,y)			//jsonCOMMAND[3][0],jsonCOMMAND[3][1]
                     更新方向： ["NP",\""+uname+"\",2,["+r+"]]	      //rotation		//jsonCOMMAND[3][0]
                     更新大小： ["NP",\""+uname+"\",3,["+s+"]]        //size			//jsonCOMMAND[3][0]
                     更新造型： ["NP",\""+uname+"\",4,["+f+","+c+"]]  //costumesfile,costumesid	//jsonCOMMAND[3][0],jsonCOMMAND[3][1]
                     更新特效： ["NP",\""+uname+"\",5,["+t+","+n+"]]  //type,number		//jsonCOMMAND[3][0],jsonCOMMAND[3][1]
         */
         var nId=arrPlayer.indexOf(jsonCOMMAND[1]);//indexOf()如果找不到，返回-1。

         if(nId>-1)//在当前项目中已经存在该选手，则直接更新选手的坐标、方向和大小数据。
         {
             switch(jsonCOMMAND[2]){
             case 1:
                oVMM.props.vm.runtime.targets[nId].setXY(jsonCOMMAND[3][0],jsonCOMMAND[3][1]);	//设置角色坐标
             break;

             case 2:
                oVMM.props.vm.runtime.targets[nId].setDirection(jsonCOMMAND[3]);		//设置角色方向
             break;

             case 3:
                oVMM.props.vm.runtime.targets[nId].setSize(jsonCOMMAND[3]);			//设置角色大小
             break;

             case 4:
                var nCID=parseInt(jsonCOMMAND[3][1])+1;
                if(arrPlayerCurrentCostume[nId]==undefined)
                {
                   arrPlayerCurrentCostume[nId]=nCID;
                   oVMM.props.vm.runtime.targets[nId].setCostume(nCID);				//设置角色造型
                }
                else if(arrPlayerCurrentCostume[nId]!=nCID)
                {
                   arrPlayerCurrentCostume[nId]=nCID;
                   oVMM.props.vm.runtime.targets[nId].setCostume(nCID);				//设置角色造型
                }
             ...
             case 5:
                oVMM.props.vm.runtime.targets[nId].setEffect(jsonCOMMAND[3][0],jsonCOMMAND[3][1]);				//设置颜色特效
             break;
...
/*
             ...
             if(arrPlayerCostume[nId]==undefined)
             {
                ...
                oVMM.props.vm.addCostume(jsonCOMMAND[3],obj,oVMM.props.vm.runtime.targets[nId].id,1);
             }
             ...
             oVMM.props.vm.runtime.targets[nId].setXY(jsonCOMMAND[2][0],jsonCOMMAND[2][1]);	//设置角色坐标
             oVMM.props.vm.runtime.targets[nId].setDirection(jsonCOMMAND[2][2]);		//设置角色方向
             oVMM.props.vm.runtime.targets[nId].setSize(jsonCOMMAND[2][3]);			//设置角色大小

             //oVMM.props.vm.runtime.targets[1].setEffect('color',46);				//设置颜色特效
...
*/
         ...
         {
            for(var i=0;i<oVMM.props.vm.runtime.targets.length ;i++){			//为确保arrPlayer中为最新数据，重新更新一下。
               arrPlayer[i]=oVMM.props.vm.runtime.targets[i].getName();			//由于添加角色操作，用时较长，且无回调消息可获取，所以只能在要用前，主动更新一下。
            }
            ...
            {
               //oVMM.props.vm.runtime.targets[nId].setXY(jsonCOMMAND[2][0],jsonCOMMAND[2][1]);
               //oVMM.props.vm.runtime.targets[nId].setDirection(jsonCOMMAND[2][2]);
               //oVMM.props.vm.runtime.targets[nId].setSize(jsonCOMMAND[2][3]);
            }
            else//没找到，还剩两种情况
            {
               if(arrPlayerDelayed.indexOf(jsonCOMMAND[1])==-1)//该选手的确没有被执行过添加操作，则添加。
               {
		  //此处数据，应该从文件获取，而非直接通过socket传递，否则会出现大文件丢数据的问题。
		  //其它操作，请参考index.php的INVITATION
                  ...
                  var strStudent=jsonCOMMAND[1];
                  $.post("loadScript.php?t=" + Math.random(), { "S":jsonCOMMAND[3] }, (function(strStudent){ return function(data){ 
                        var jsondata=JSON.parse(data); 
                        jsondata["name"]=strStudent;
                        ...
                        jsondata["blocks"]={"[Hk%KZ[nBuUB@J-g8:by":{"opcode":"event_whenflagclicked","next":"pj7L#^k03uy5V{tVed9a","parent":null,"inputs":{},"fields":{},"shadow":false,"topLevel":true,"x":59,"y":145},"pj7L#^k03uy5V{tVed9a":{"opcode":"looks_say","next":null,"parent":"[Hk%KZ[nBuUB@J-g8:by","inputs":{"MESSAGE":[1,[10,strStudent]]},"fields":{},"shadow":false,"topLevel":false}};//{};//将学生名字显示在舞台上。
                        oVMM.props.vm.addSprite(jsondata); 
                   }})(strStudent) );

/*
                  var objSprite = jsonCOMMAND[3];
                  oVMM.props.vm.addSprite(objSprite);						//当前插入一辆小车的角色
                  oVMM.props.vm.runtime.targets[nId].setXY(jsonCOMMAND[2][0],jsonCOMMAND[2][1]);//同步更新坐标
                  oVMM.props.vm.runtime.targets[nId].setDirection(jsonCOMMAND[2][2]);		//同步更新方向
                  oVMM.props.vm.runtime.targets[nId].setSize(jsonCOMMAND[2][3]);		//同步更新大小
*/
               }
      ...
      case "GREENFLAG":
         bCodeViewableFile = false;							//强制置播放模式
         updateScreenMode();
         oVMM.props.vm.runtime.greenFlag();				//远程执行
      ...
      case "BACKDROP2":						//添加URL中图片为背景的指令：["BACKDROP2","./fileurl"]
         $.get("addBackdrop2.php?f="+jsonCOMMAND[1]+"&t=" + Math.random(), function (data)//获取文件URL
         {
            //var vmBackdrop = {name: "Arctic", rotationCenterX: 480,rotationCenterY: 360, bitmapResolution: 2, skinId: null};
            var jsonData=$.parseJSON(data);

            oVMM.props.vm.addBackdrop(jsonData["md5"],jsonData);
         });
         break;
      ...
      case "BACKDROPX":						//他人共享过来的角色：["SPRITEX","Script File MD5"]
         ...
         {
            $.post("loadScript.php?t=" + Math.random(), { "S":jsonCOMMAND[3] }, (function(o){ return function(data){ //共享数据保存在文件中。
               var jsondata=JSON.parse(data); 

               for(i=0;i<jsondata.costumes.length;i++){			//接收分享过来的舞台背景数据
                  dCostume=jsondata.costumes[i];

                  if(jsondata.costumes[i].asset.data.length==undefined)//非矢量图
                  {
...
<script src=./js/controlt.js></script>
