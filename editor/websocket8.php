...
class WebSocket {
    ...
    const LISTEN_SOCKET_NUM = 1000;	//最大监听数量

    /**
     * @var array $sockets
     *    (
     *      spl_object_id($socket) => Array(				//PHP >= 8:sql_object_id($socket)    //PHP < 8 (int)$socket
     *			           'resource' => nSocketId,
     *       			   'uname' => strUsername,
...
*/
     ...
     * @var array $sockets
     *    (
     *      (String)$username =>  spl_object_id($socket)
     *    )
     *  用户名与socket对应
     */
        ...
        foreach ($sockets as $socket) {						//如有Socket的状态发生了变化，则处理
            if ($socket == $this->master) {					//  主进程的状态发生了变化，则表示当前有新的接入请求出现
                ...
                if ($bytes < 9) {						//数据量太少，遗弃，并终止连接。
                    ...
                    if (!$this->sockets[spl_object_id($socket)]['handshake']) {		//如该连接未曾握过手，则对接收到的数据执行握手操作
...
//print_r($this->User2Socket);

             if(isset($this->User2Socket[$msg_content])) 
             {
                // echo "RELOGIN\n\n\n";
                 //socket_close($socket);

                $user_list = array_column($this->sockets, 'uname');	//获取当前用户列表
                $response['type'] = 'RELOGIN';				
             ...
             {
                $this->User2Socket[$msg_content]=1;

                //print_r($this->User2Socket);


                $this->sockets[spl_object_id($socket)]['uname'] = $msg_content;   //登录成功，更新当前socket句柄所对应的用户名
...
//echo "LST:".$this->lastBCMsg."\n\n\n";
            ...
            case 'logout':
                ...
                unset($this->User2Socket[$msg_content]);
//echo $msg_content;
//echo "LOGOUT\n";						
                $user_list = array_column($this->sockets, 'uname');	//获取当前用户列表
//echo "USERLIST:".count($user_list)."\r\n";
		...
		//该用户在sockets和User2Socket中的数据，已经在disconnect()中被删除了，所以不需要再次操作。
                ...
                break;
            case 'user':						//由教师主导发布的广播信息，以及学生受限制地发布的广播信息（速问速答的答题操作）
                $BCType=1;
                print_r($recv_msg);
                //echo "[[".$msg_id."]]\n";
                $uname = $this->sockets[spl_object_id($socket)]['uname'];
                ...
                break;
            case 'chat':						//扩展“聊天室”的广播消息
                $BCType=1;
                //print_r($recv_msg);
                //echo "[[".$msg_id."]]\n";
                $uname = $this->sockets[spl_object_id($socket)]['uname'];
                $response['type'] = 'chat';				//数据类型：用户通信
                $response['from'] = $uname;				//发送者
                $response['content'] = $msg_content;			//发送内容
                $response['to'] = $msg_to;				//指定接收者。
                $response['lastID'] = md5($msg_content);
    ...
    public function connect($socket) {
        ...
        $this->sockets[spl_object_id($socket)] = $socket_info;				//将Socket的配置信息与Socket文件句柄关联起来，保存到$this->sockets中。
    ...
    private function disconnect($socket) {
        $recv_msg = Array(
            'type' => 'logout',
            'content' => isset($this->sockets[spl_object_id($socket)]['uname'])?$this->sockets[spl_object_id($socket)]['uname']:"",
        );
        unset($this->User2Socket[$recv_msg['content']]);			//删除从用户名到socket句柄的映射关系。
        unset($this->sockets[spl_object_id($socket)]);					//从sockets池中删除该socket句柄。
        if($socket) socket_close($socket);							//关闭该socket连接
    ...
    public function handShake($socket, $buffer) {
        ...
        $this->sockets[spl_object_id($socket)]['handshake'] = true;			//更新握手状态
...
echo $TNAME;
$ws = new WebSocket("0.0.0.0", $port, $TNAME);