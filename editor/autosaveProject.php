...
if($classid!="" && $room && $date!="" && $username!="" && isset($classname[$classid-1]))
{
   if($projname==""){
   ...
   //if(!file_exists("../".$student_works."/".$classname[$classid-1])) mkdir("../".$student_works."/".$classname[$classid-1]);
   ...
   { 
      ...
      if( !file_exists($path."/backup/".$ip."_".$username."_$projname"))  mkdir($path."/backup/".$ip."_".$username."_$projname",0744,true);

      $savedfile=date("Ymd_His",time());
