...
$task_name  =isset($_POST["T"])?$_POST["T"]:exit();
...
if(isset($teacher_room[$ip]))	//教师访问，强制用IP控制。
{
   ...
   $lesson_name=@file_get_contents($path."lesson.dat");

   if($lesson_name)
   {
      if(!file_exists($path.$lesson_name."/".$task_name))
      {
         if(mkdir($path.$lesson_name."/".$task_name,0777,true))
         {
            echo "OK";
         }
         file_put_contents($path.$lesson_name."/task.dat",$task_name);
