...
var strMsgSendPlay = '';
var heartBeatTimePlay = 60;				//一分钟一次
var urlPlay='';
var strUSER='';
var bPlayReady=false;
/**
 * 分析服务器返回信息
 *
...
function reconnectPlay(url) {
    ...
    setTimeout(function () {     			//没连接上会一直重连，设置延迟避免请求过多
       if(wsPlay)
       {
          if( wsPlay.readyState==1)
          {
              wsPlay.close();
          }

           wsPlay.onclose=null;				//清除连接失败的WebSocket的各类回显响应
           wsPlay.onerror=null;				//否则，当再次连接上后，会有N个连接成功的消息，
           wsPlay.onopen=null;				//其实是旧连接没有被清理掉。
           wsPlay.onmessage=null;
           wsPlay=null;
        }
    ...
    wsPlay.onmessage = function (msg) {    			//如果获取到消息，心跳检测重置
        ...
        switch (msg.type) {
            ...
            case 'login':
                ...
                if(strUSER=="Stage" && bPlayReady==false) oVIEW.props.setFullScreen(true);
                bPlayReady=true;
...
var heartCheckPlay = {
    ...
    start: function () { 				// 开启定时
        var self = this;
        this.timeoutObj = setInterval(function () { 	// 心跳时间内收不到消息，主动触发连接关闭，开始重连
            if(wsPlay==null || wsPlay.readyState!=1)
               wsPlay.close();
        }, this.timeout)
    }
}
