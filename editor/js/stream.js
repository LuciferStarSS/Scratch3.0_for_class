...
function initEventHandle() {
    ...
    ws.onerror = function (err) {
        ...
        var d = new Date();
        var n = d.toLocaleTimeString();
        var data = "系统消息 (" + n + ")+: 出错了，正在尝试重新运行服务，请稍候。";
        listMsg(data);
        reconnect.lockReconnect = false;
        reconnect(strWebSocketServerURL);
    };
...
function add(name) {
	...
	strMsgSend =name;
        ...
        var msg = to==''?  { 'content': strMsgSend , 'type': 'user'  ,'to':''} : { 'content': strMsgSend , 'type': 'user'  ,'to':to};
        sendMsg(msg);
    }
}
...
function dealUser(user_name, type, name_list) {
    ...
    var data = "系统消息(" + n + "): " + user_name + " 已" + change;
    listMsg(data);
}
...
{
//   alert(o.innerText);
   document.getElementById("to").value=o.innerText;
}
/**
 * 将数据转为json并发送
 * @param msg
