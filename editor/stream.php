...
class WebSocket {
    ...
    public function __construct($host, $port) {				     //构造函数，初始化
        try {
            ...
            socket_bind($this->master, $host, $port);			     // listen函数使用主动连接套接口变为被连接套接口，使得一个进程可以接受其它进程的请求，从而成为一个服务器进程。在TCP服务器编程中listen函数把进程变为一个服务器，并指定相应的套接字变为被动连接,其中的能存储的请求不明的socket数目。
        ...
        while (true) {
            try {
                $this->doServer();
                $signal=@file_get_contents("./c.dat");
echo $signal."\r\n{}";
                if($signal=="1") exit("Exit Now.");
            } catch (Exception $e) {
                $this->error([
                    'error_do_server',
                    $e->getCode(),
    ...
    private function doServer() {						//服务端监听，当有连接请求时，记录连接的socket信息。
        $write = $except = NULL;
        ...
        foreach ($sockets as $socket) {					// 遍历线程池
            if ($socket == $this->master) {
                $client = socket_accept($this->master);			// 创建,绑定,监听后accept函数将会接受socket要来的连接,一旦有一个连接成功,将会返回一个新的socket资源用以交互,如果是一个多个连接的队列,只会处理第一个,如果没有连接的话,进程将会被阻塞,直到连接上.如果用set_socket_blocking或socket_set_noblock()设置了阻塞,会返回false;返回资源后,将会持续等待连接。
                if (false === $client) {
                    $this->error([
                        'err_accept',
                        $err_code = socket_last_error(),
                ...
                $msg = self::dealMsg($socket, $recv_msg);		//处理每个用户的消息
    ...
    private function dealMsg($socket, $recv_msg) {
        ...
        $msg_to = isset($recv_msg['to'])?$recv_msg['to']:'';
        $response = [];
print_r($recv_msg);
        switch ($msg_type) {
            case 'login':
...
//                $user_list = array_filter(array_column($this->sockets, 'uname'));
    ...
    private function broadcast($data) {
        foreach ($this->sockets as $socket) {
            if ($socket['resource'] != $this->master) {				//直接排除主监听socket，不向自己发送消息。
print_r($socket);
